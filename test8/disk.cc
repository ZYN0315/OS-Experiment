...
DiskArm::DiskArm() {
    ...
    for (i = 0; i < RequestNumber; i++) cin >> Request[i];
}
DiskArm::~DiskArm() {}
//初始化道号，寻道记录
void DiskArm::InitSpace(char *MethodName) {
    ...
    SeekNumber = 0;
    SeekChang = 0;
    for (i = 0; i < RequestNumber; i++) Cylinder[i] = Request[i];
}
// 统计报告算法执行情况
...
void DiskArm::Scan(void) {
    ...
    for (int i = 0; i < RequestNumber; i++) {
        ...
        for (int j = 0; j < RequestNumber; j++) {
            if (Cylinder[j] == -1)
                continue;
            else if ((Direction == 0 && Cylinder[j] < Current &&
                      (Current - Cylinder[j]) < Distance) ||
                     (Direction == 1 && Cylinder[j] > Current &&
                      (Cylinder[j] - Current) < Distance)) {
                index = j;
                Distance = abs(Current - Cylinder[j]);
            }
        }
