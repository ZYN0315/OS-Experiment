...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (sofa_count < 4) {
            ...
            if (msgsnd(sofa_quest_id, &msg_arg, sizeof(msg_arg),
                       sofa_quest_flg) >= 0) {
            }
            sofa_count++;
            ...
            printf(
                "waiting room is full %d customer can't get into barber shop\n",
                i);
        }
        sofa_quest_flg = IPC_NOWAIT;
        if (msgrcv(sofa_respond_id, &msg_arg, sizeof(msg_arg), 0,
                   sofa_quest_flg) >= 0) {
            sofa_count--;
        }
        wait_quest_flg = IPC_NOWAIT;
        if (msgrcv(wait_respond_id, &msg_arg, sizeof(msg_arg), 0,
                   wait_quest_flg) >= 0) {
            wait_count--;
        }
    }
